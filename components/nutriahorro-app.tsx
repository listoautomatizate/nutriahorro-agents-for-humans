...
function ChatDrawer({ messages, input, setInput, send, close, busy }: { messages: ChatMessage[]; input: string; setInput: (value: string) => void; send: (value?: string, confirmedAction?: AgentAction) => void; close: () => void; busy: boolean }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}><aside className="chat-drawer"><header><div><span><Leaf size={19} /></span><div><strong>Agente nutrIAhorro</strong><small>Activo con memoria de tu despensa</small></div></div><button className="row-icon-button" onClick={close} title="Cerrar" type="button"><X size={20} /></button></header><div className="chat-body">{messages.map((item, index) => <div className={`chat-message ${item.role}`} key={`${item.role}-${index}`}><span>{item.text}</span>{item.role === 'agent' && item.tools?.length ? <small className="tool-trace"><Sparkles size={11} /> {item.mode?.startsWith('strands-') ? 'Strands Agents' : 'Respaldo local'} · Herramientas: {item.tools.join(', ')}</small> : null}{item.role === 'agent' && item.action ? <button className="chat-action-button" disabled={busy} onClick={() => send(`Confirmo que cocine ${item.action?.recipe_name}. Registrala ahora con recipe_id ${item.action?.recipe_id} y confirmed=true.`, item.action)} type="button"><Check size={15} /> Confirmar comida</button> : null}</div>)}{busy && <div className="chat-message agent typing"><span /><span /><span /></div>}<div ref={endRef} /></div><div className="quick-prompts">{quick.map((item) => <button key={item} onClick={() => send(item)} type="button">{item}</button>)}</div><form className="chat-form" onSubmit={(event) => { event.preventDefault(); send(); }}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Preguntale algo" /><button disabled={!input.trim() || busy} title="Enviar" type="submit"><Send size={18} /></button></form></aside></div>;
}
