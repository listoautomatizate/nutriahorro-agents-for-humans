...
export async function POST(request: Request) {
  try {
    ...
    if (!body.name?.trim() || !body.category?.trim() || !body.unit?.trim() || Number(body.quantity) <= 0) {
      return Response.json({ error: 'Enter a name, category, quantity, and unit.' }, { status: 400 });
    }
    ...
    return Response.json({ error: error instanceof Error ? error.message : 'The food could not be saved.' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return Response.json({ error: 'No food item was selected.' }, { status: 400 });
    await deletePantryItem(id);
    return Response.json(await getAppState());
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : 'The food could not be removed.' }, { status: 500 });
  }
}
